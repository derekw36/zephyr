...
const struct device *const IRC_OSC = DEVICE_DT_GET_OR_NULL(LPC17XX_IRC_OSC);
...
BUILD_ASSERT(LPC17XX_IRC_OSC_ENABLED, "irc_osc cannot be disabled.");

// Internal oscillator frequency must match LPCOpen definition
BUILD_ASSERT(LPC17XX_IRC_OSC_FREQ == SYSCTL_IRC_FREQ, "irc_osc clock frequency is invalid.");

#if LPC17XX_RTC_CLK_ENABLED
// RTC oscillator must be 32.768 kHz
BUILD_ASSERT(LPC17XX_RTC_CLK_FREQ == 32768, "rtc_clk clock frequency must be 32.768 kHz.");
#endif
...
static __maybe_unused void lpc17xx_clock_set_mux_usbclk(const struct device *clk)
{
	if (clk == NULL) {
		return;
	}
...
static int lpc17xx_pll_on(const struct device *dev, __unused clock_control_subsys_t sys)
{
...
static int lpc17xx_pll_off(const struct device *dev, __unused clock_control_subsys_t sys)
{
	const struct lpc17xx_pll_config *config = dev->config;
...
static enum clock_control_status lpc17xx_pll_get_status(const struct device *dev,
							__unused clock_control_subsys_t sys)
{
	...
	if (locked) return CLOCK_CONTROL_STATUS_ON;
	else if (enabled) return CLOCK_CONTROL_STATUS_STARTING;
	else return CLOCK_CONTROL_STATUS_OFF;
}

static int lpc17xx_pll_get_rate(const struct device *dev, __unused clock_control_subsys_t sys,
				uint32_t *rate)
{
	const struct lpc17xx_pll_config *config = dev->config;
...
static __maybe_unused int lpc17xx_pll_init(const struct device *dev)
{
	const struct lpc17xx_pll_config *config = dev->config;
...
// Main PLL source clock must be one of irc_osc, rtc_clk or osc_clk
BUILD_ASSERT(DT_SAME_NODE(DT_CLOCKS_CTLR(LPC17XX_MAIN_PLL), LPC17XX_IRC_OSC) ||
	     DT_SAME_NODE(DT_CLOCKS_CTLR(LPC17XX_MAIN_PLL), LPC17XX_RTC_CLK) ||
	     DT_SAME_NODE(DT_CLOCKS_CTLR(LPC17XX_MAIN_PLL), LPC17XX_OSC_CLK),
	     "main_pll source clock must be one of irc_osc, rtc_clk, or osc_clk.");

// Main PLL source clock device must be enabled
BUILD_ASSERT(DT_NODE_HAS_STATUS_OKAY(DT_CLOCKS_CTLR(LPC17XX_MAIN_PLL)));
...
// USB PLL source clock must be osc_clk
BUILD_ASSERT(DT_SAME_NODE(DT_CLOCKS_CTLR(LPC17XX_USB_PLL), LPC17XX_OSC_CLK),
	     "usb_pll source clock must be osc_clk.");

// USB PLL source clock device must be enabled
BUILD_ASSERT(DT_NODE_HAS_STATUS_OKAY(DT_CLOCKS_CTLR(LPC17XX_USB_PLL)));
...
// Clock controller source clock must be one of irc_osc, rtc_clk, osc_clk, or main_pll
BUILD_ASSERT(DT_SAME_NODE(DT_CLOCKS_CTLR(LPC17XX_CLOCK_CONTROL), LPC17XX_IRC_OSC) ||
	     DT_SAME_NODE(DT_CLOCKS_CTLR(LPC17XX_CLOCK_CONTROL), LPC17XX_RTC_CLK) ||
	     DT_SAME_NODE(DT_CLOCKS_CTLR(LPC17XX_CLOCK_CONTROL), LPC17XX_OSC_CLK) ||
	     DT_SAME_NODE(DT_CLOCKS_CTLR(LPC17XX_CLOCK_CONTROL), LPC17XX_MAIN_PLL),
	     "clkc source clock must be one of irc_osc, rtc_clk, osc_clk, or main_pll.");

// Clock controller source clock device must be enabled
BUILD_ASSERT(DT_NODE_HAS_STATUS_OKAY(DT_CLOCKS_CTLR(LPC17XX_CLOCK_CONTROL)));
